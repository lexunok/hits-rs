...
pub fn skill_router() -> Router<AppState> {
    Router::new()
        .route(
            "/",
            get(get_all_skills).post(create_skill).put(update_skill),
        )
        .route("/{user_id}", get(get_all_user_skills))
...
async fn get_all_user_skills(
    State(state): State<AppState>,
    _: Claims,
    Path(user_id): Path<Uuid>,
) -> Json<Vec<SkillDto>> {
    let skills = SkillService::get_all_by_user(&state, user_id).await;
    Json(skills)
}
