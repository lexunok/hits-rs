...
- **`GET /users/{id} - {id} Optional`**
- **Описание:** Возвращает детальную информацию о пользователе, если не передать id то будет текущий пользователь.
...
### Создание компании
- **`POST /company`**
- **Описание:** Создает новую компанию. Требует прав администратора.
...
### Обновление компании
- **`PUT /company`**
- **Описание:** Обновляет данные компании по ID. Требует прав администратора.
...
- **Возможные ошибки:**
  - **`401 Unauthorized`**: Требуется аутентификация.
  - **`404 Not Found`**: Компания с указанным `id` не найдена.

### Удаление компании
- **`DELETE /company/{id}`**
- **Описание:** Удаляет компанию. Требует прав администратора.
- **Ответ (`200 OK`, `MessageResponse`):**
  ```json
  {
...
- **`GET /market/active`**
- **Описание:** Возвращает список всех маркетов со статусом `ACTIVE`.
- **Права доступа:** Требуется аутентификация.
- **Ответ (`200 OK`, `Vec<MarketDto>`):** (Аналогично `GET /market`, но только с `status: "ACTIVE"`)
- **Возможные ошибки:**
  - **`401 Unauthorized`**: Требуется аутентификация.
...
- **Возможные ошибки:**
  - **`401 Unauthorized`**: Требуется аутентификация.
  - **`404 Not Found`**: Маркет с указанным `id` не найден.

### Создание маркета
- **`POST /market`**
- **Описание:** Создает новый маркет.
- **Права доступа:** `Admin`, `ProjectOffice`.
- **Тело запроса (`CreateMarketRequest`):**
  ```json
  {
    "name": "New Awesome Market",
    "start_date": "2026-03-01",
    "finish_date": "2026-04-01"
  }
  ```
- **Ответ (`200 OK`, `MarketDto`):** Возвращает созданный маркет.
- **Возможные ошибки:**
  - **`403 Forbidden`**: Нет прав администратора или проектного офиса.

### Обновление маркета
- **`PUT /market`**
- **Описание:** Обновляет данные маркета по ID.
- **Права доступа:** `Admin`, `ProjectOffice`.
- **Тело запроса (`UpdateMarketRequest`):**
  ```json
  {
    "id": "market-uuid-to-update",
...
- **Возможные ошибки:**
  - **`403 Forbidden`**: Нет прав администратора или проектного офиса.
  - **`404 Not Found`**: Маркет не найден.

### Обновление статуса маркета
- **`PUT /market/status`**
- **Описание:** Обновляет статус маркета.
- **Права доступа:** `Admin`, `ProjectOffice`.
- **Тело запроса (`UpdateMarketStatusRequest`):**
  ```json
  {
    "id": "market-uuid-to-update",
    "status": "ACTIVE"
